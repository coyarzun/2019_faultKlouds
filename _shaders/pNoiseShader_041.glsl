...
uniform float mode;
uniform float pCycles;
uniform sampler2D ppixels;
uniform sampler2D micIn;
uniform float doCurves;
uniform float doLGlitch;
uniform float doRGlitch;
uniform float leftGlitch;
uniform float rightGlitch;
uniform float wichShadeVariation;
...
void main(void) {
  ...
  float n = fbm(-time*0.01+rm*1.15);

  float stime = n*1.0;//*(1.0+cos(a+time*0.01));

////////////////////////

            if(p.x<leftGlitch && doLGlitch==1.0){
                p.x = leftGlitch;
            }
            if(p.x>rightGlitch && doRGlitch==1.0){
                p.x = rightGlitch;
            }
...
////////////////da color
    ...
    vec4 pcolor       = texture2D(texture, p);

  vec2 rmx = vec2(r,r);

  float rr    = color.x+noise(stime*1.01+rmx*1.0);
  float bb    = color.y-noise(-stime*10.025+rmx*1.0);
  float gg    = color.z-noise(stime*10.1-rmx*1.0);
...
*/
    //////////////////////
      float h = rgb2hsv(vec3(color.rgb)).g;
    h = mod(h+time,1.0);

  gl_FragColor =  vec4((vec3(2.0*rr,6.0*bb,8.0*gg)),1.0)*vec4(hsv2rgb(vec3(h,1.0,1.0)), 1.0);
  //vec4(color);//hsv2rgb//+gl_FragColor/1.0+hsv2rgb
}
